        ...
        def view_all(ans):
            ...
            if(ans == options[0]):
                ...
                if(interact == ao2[0]):
                    ...
                    print('\n')
                    print(f'Most Accumulated Cost ($) for a habit: {Analytics.most_expensive(habits)}')
                    print(f'Most Fails for a habit: {Analytics.most_fail(habits)}')
                    print(f'Most Success for a habit: {Analytics.most_success(habits)}')
                    print('\n')
                    print(f'Total Fail: {Analytics.total_fail(habits)}')
                    print(f'Total Success: {Analytics.total_success(habits)}')
                    print('\n')
                    print(f'Best performing interval: {Analytics.best_performing_interval(habits)}')
                    print(f'Best performing category: {Analytics.best_performing_category(habits)}')
                    print('\n')
                    print(f'Max Current Streak: {Analytics.total_longest_current_streak(habits)}')
                    print(f'Max Running Streak: {Analytics.total_longest_running_streak(habits)}')
                    # print(f'most_punctual_sec:  {Analytics.most_punctual(habits)}')
                    # print(f'most_late_sec: {Analytics.most_late(state["active_user"].habits)}')
                    print(f'Average Total Streak: {round(Analytics.avg_total_streak(habits),2)}')
                    print(f'Average Break Streak: {round(Analytics.avg_break_streak(habits),2)}')
                    print(f'Average Time Left Till Deadline: {seconds_to_timestring(round(Analytics.avg_time_left(habits),2))}')
                    print('\n')
                    print(f'Earliest Habit Deadline: {Analytics.earliest(habits).title}')
                    print('\n')
